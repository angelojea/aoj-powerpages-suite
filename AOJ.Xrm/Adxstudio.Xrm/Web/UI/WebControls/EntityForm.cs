...
namespace Adxstudio.Xrm.Web.UI.WebControls
{
	...
	public class EntityForm : CompositeControl
	{
		...
		public MappingFieldMetadataCollection MappingFieldCollection
		{
			get
			{
				if (_mappingFieldCollection == null)
				{
					...
					var attributes = new[]
					{
						new FetchAttribute("mspp_geolocation_addresslinefieldname"), new FetchAttribute("mspp_geolocation_cityfieldname"),
						new FetchAttribute("mspp_geolocation_countryfieldname"), new FetchAttribute("mspp_geolocation_countyfieldname"),
						new FetchAttribute("mspp_geolocation_formattedaddressfieldname"), new FetchAttribute("mspp_geolocation_latitudefieldname"),
						new FetchAttribute("mspp_geolocation_longitudefieldname"), new FetchAttribute("mspp_geolocation_neighborhoodfieldname"),
						new FetchAttribute("mspp_geolocation_postalcodefieldname"), new FetchAttribute("mspp_geolocation_statefieldname"),
					...
					var mappingFieldCollection = new MappingFieldMetadataCollection
					{
						...
						CountryFieldName = entityform.GetAttributeValue<string>("mspp_geolocation_countryfieldname"),
						CountyFieldName = entityform.GetAttributeValue<string>("mspp_geolocation_countyfieldname"),
						FormattedLocationFieldName = entityform.GetAttributeValue<string>("mspp_geolocation_formattedaddressfieldname"),
						LatitudeFieldName = entityform.GetAttributeValue<string>("mspp_geolocation_latitudefieldname"),
						LongitudeFieldName = entityform.GetAttributeValue<string>("mspp_geolocation_longitudefieldname"),
						NeightbourhoodFieldName = entityform.GetAttributeValue<string>("mspp_geolocation_neighborhoodfieldname"),
						PostalCodeFieldName = entityform.GetAttributeValue<string>("mspp_geolocation_postalcodefieldname"),
						StateProvinceFieldName = entityform.GetAttributeValue<string>("mspp_geolocation_statefieldname"),
						Enabled = entityform.GetAttributeValue<bool>("mspp_geolocation_enabled"),
						DisplayMap = entityform.GetAttributeValue<bool>("mspp_geolocation_displaymap")
					};
        ...
        protected override void CreateChildControls()
		{
			...
			//{
			//	ADXTrace.TraceInfo(TraceCategory.Application, "The current entity must be of type mspp_webpage. Please select the correct template for this entity type.");
			//	return;
			//}
			//var entityformReference = sitemapNodeEntity.GetAttributeValue<EntityReference>("mspp_entityform");
			//if (entityformReference == null)
			//{
			//	ADXTrace.TraceInfo(TraceCategory.Application, "CreateChildControls", string.Format("Could not find an Entity Form (mspp_entityform) value on Web Page (mspp_webpage) where id equals {0}.", sitemapNodeEntity.Id));
			//	return;
			//}
			//entityform = context.RetrieveSingle("mspp_entityform", "mspp_entityformid", entityformReference.Id, FetchAttribute.All);
			//if (entityform == null)
			//{
			//	ADXTrace.TraceError(TraceCategory.Application, string.Format("Could not find an Entity Form (mspp_webpage_entityform) value where id equals {0} on Web Page (mspp_webpage) where id equals {1}.", entityformReference.Id, sitemapNodeEntity.Id));
			//	return;
			//}
			...
			EnableEntityPermissions = entityform.GetAttributeValue<bool?>("mspp_entitypermissionsenabled").GetValueOrDefault(false);

			var modeOption = entityform.GetAttributeValue<OptionSetValue>("mspp_mode");
			...
			var recordNotFoundMessage = Localization.GetLocalizedString(entityform.GetAttributeValue<string>("mspp_recordnotfoundmessage"), LanguageCode);
		...
		public CompositeControl RenderForm(OrganizationServiceContext context, Entity entityform, FormViewMode mode, FormEntitySourceDefinition entitySourceDefinition)
		{
			...
			var formMetadataJson = entityform.GetAttributeValue<string>("mspp_settings");
			...
			var entityName = entityform.GetAttributeValue<string>("mspp_entityname");
		...
		{
			entityform.AssertEntityName("mspp_entityform");
			AssociateToCurrentPortalUserOnItemInserted = false;

			string id = Guid.Empty.ToString();
			var logicalName = entityform.GetAttributeValue<string>("mspp_entityname");
			var primaryKey = entityform.GetAttributeValue<string>("mspp_primarykeyname");

			if (string.IsNullOrWhiteSpace(logicalName))
			{
				throw new ApplicationException("mspp_entityform.mspp_entityname must not be null.");
			}
			...
			var entitySourceType = entityform.GetAttributeValue<OptionSetValue>("mspp_entitysourcetype");
			...
			switch (entitySourceType.Value)
			{
				case 756150001: // Query String
					var queryStringParameter = entityform.GetAttributeValue<string>("mspp_recordidquerystringparametername");
				...
				case 756150003: // Record Associated to Current Portal User
					var relationship = entityform.GetAttributeValue<string>("mspp_recordsourcerelationshipname");
					...
					if (source == null)
					{
						var allowCreate = entityform.GetAttributeValue<bool?>("mspp_recordsourceallowcreateonnull").GetValueOrDefault();
						if (allowCreate)
						{
					...
					break;
				default:
					throw new ApplicationException("mspp_entityform.mspp_entitysourcetype is not valid."); 
			}
		...
		protected void OnItemUpdating(object sender, CrmEntityFormViewUpdatingEventArgs e)
		{
			...
			if (entityform == null)
			{
				throw new ApplicationException(string.Format("Error retrieving mspp_entityform where id equals {0}", EntityFormReference.Id));
			}
		...
		protected void OnItemInserted(object sender, CrmEntityFormViewInsertedEventArgs e)
		{
			...
			if (e.Exception == null)
			{
				...
				{
					ADXTrace.TraceError(TraceCategory.Application, "Saving record failed. EntityId is null. This error usually indicates a plugin has failed or a system required field was not provided a value. Please check the system jobs in CRM for possible failed plugin steps.");
				}
				else
				{
					if (AssociateToCurrentPortalUserOnItemInserted)
					{
						...
						var relationshipName = entityform.GetAttributeValue<string>("mspp_recordsourcerelationshipname");
			...
			{
				ADXTrace.TraceError(TraceCategory.Application, string.Format("Saving record failed. EntityId is null. This error usually indicates a plugin has failed or a system required field was not provided a value. Please check the system jobs in CRM for possible failed plugin steps. {0}", e.Exception.InnerException));
			}
		...
		protected void OnItemUpdated(object sender, CrmEntityFormViewUpdatedEventArgs e)
		{
			...
			if (e.Exception == null)
			{
				...
				{
					ADXTrace.TraceError(TraceCategory.Application, "Saving record failed. Entity.Id is null. This error usually indicates a plugin has failed or a system required field was not provided a value. Please check the system jobs in CRM for possible failed plugin steps.");
				}
				else
				{
			...
			{
				ADXTrace.TraceError(TraceCategory.Application, string.Format("Saving record failed. Entity.Id is null. This error usually indicates a plugin has failed or a system required field was not provided a value. Please check the system jobs in CRM for possible failed plugin steps. {0}", e.Exception.InnerException));
			}
		...
		{
			var onSuccessOption = entityform.GetAttributeValue<OptionSetValue>("mspp_onsuccess");
		...
		protected void ProcessRedirect(OrganizationServiceContext context, Entity entityform, FormEntitySourceDefinition entitySourceDefinition)
		{
			...
			var redirectUrl = entityform.GetAttributeValue<string>("mspp_redirecturl");
			var appendExistingQueryString = entityform.GetAttributeValue<bool?>("mspp_appendquerystring") ?? false;
			UrlBuilder url;

			if (string.IsNullOrWhiteSpace(redirectUrl))
			{
				var pageReference = entityform.GetAttributeValue<EntityReference>("mspp_redirectwebpage");

				if (pageReference == null)
				{
					ADXTrace.TraceError(TraceCategory.Application, "mspp_entityform.mspp_entityform_redirectwebpage is null");
					return;
				}
				...
				var page = context.RetrieveSingle(
					...
					FetchAttribute.None,
					new Condition("mspp_webpageid", ConditionOperator.Equal, pageReference.Id));

				if (page == null)
				{
					ADXTrace.TraceError(TraceCategory.Application, string.Format("Could not find web page with id equal to '{0}'", pageReference.Id));
					return;
				}
				...
				if (path == null)
				{
					ADXTrace.TraceError(TraceCategory.Application, "mspp_entityform.mspp_entityform_redirectwebpage URL is null");
					return;
				}
			...
			var addquerystring = entityform.GetAttributeValue<bool?>("mspp_redirecturlappendentityidquerystring") ?? false;

			if (addquerystring)
			{
				var queryStringParameterName = entityform.GetAttributeValue<string>("mspp_redirecturlquerystringname");

				if (entitySourceDefinition.ID == Guid.Empty)
				{
					ADXTrace.TraceError(TraceCategory.Application, "Record ID not appended to query string. ID is null.");
				}
				else
				{
			...
			var customQueryString = entityform.GetAttributeValue<string>("mspp_redirecturlcustomquerystring");
			...
			if (!string.IsNullOrWhiteSpace(customQueryString))
			{
				...
				{
					ADXTrace.TraceError(TraceCategory.Application, string.Format("Failed to add Custom Query String (mspp_redirecturlcustomquerystring) to the Query String. {0}", ex));
				}
			}

			var queryStringAttributeParameterName = entityform.GetAttributeValue<string>("mspp_redirecturlquerystringattributeparamname");

			if (!string.IsNullOrWhiteSpace(queryStringAttributeParameterName))
			{
				var queryStringAttributeLogicalName = entityform.GetAttributeValue<string>("mspp_redirecturlquerystringattribute");
				...
				if (!string.IsNullOrWhiteSpace(queryStringAttributeLogicalName))
				{
					if (entitySourceDefinition.ID != Guid.Empty && !string.IsNullOrWhiteSpace(entitySourceDefinition.LogicalName) && !string.IsNullOrWhiteSpace(entitySourceDefinition.PrimaryKeyLogicalName))
					{
						...
						if (record == null)
						{
							ADXTrace.TraceError(TraceCategory.Application, string.Format("Failed to add attribute value to the Query String. Record could not be found with ID equal to '{0}'", entitySourceDefinition.ID));
						}
						else
						{
							if (record.Attributes.ContainsKey(queryStringAttributeLogicalName))
							{
								...
								if (queryStringAttributeValue != null)
								{
									...
									{
										ADXTrace.TraceError(TraceCategory.Application, "Could not add attribute value to the Query String. Value is null.");
									}
								}
								else
								{
									ADXTrace.TraceError(TraceCategory.Application, "Failed to add attribute value to the Query String. Attribute value does not exist.");
								}
							}
							else
							{
								ADXTrace.TraceError(TraceCategory.Application, string.Format("Failed to add attribute value to the Query String. Record could not be found or entity does not contain attribute '{0}'", queryStringAttributeLogicalName));
							}
						}
					}
					else
					{
						ADXTrace.TraceError(TraceCategory.Application, "Failed to add attribute value to the Query String. entitySourceDefinition is not valid.");
					}
				}
			}
		...
		protected bool TrySetLookupFieldValue(OrganizationServiceContext context, Control container, EntityReference value, string attributeName)
		{
			try
			{
				...
				{
					ADXTrace.TraceError(TraceCategory.Application, string.Format("'{0}' field is not one of the expected control types.", attributeName));
			...
			{
				ADXTrace.TraceError(TraceCategory.Application, string.Format("'{0}' field value could not be set. {1}", attributeName, e));
		...
		protected void SetAttributeValuesOnSave(OrganizationServiceContext context, Entity entityform, object e)
		{
				...
				filters: new[]
						{
							new Filter
							{
								Conditions = new[]
								{
									new Condition("mspp_setvalueonsave", ConditionOperator.Equal, true)
								}
							}
						}).Entities;
			...
			var targetEntityLogicalName = entityform.GetAttributeValue<string>("mspp_entityname");

			if (string.IsNullOrWhiteSpace(targetEntityLogicalName))
			{
				throw new ApplicationException("mspp_entityform.mspp_entityname is null.");
			}
			...
			foreach (var item in metadata)
			{
				...
				if (string.IsNullOrWhiteSpace(attributeName))
				{
					ADXTrace.TraceError(TraceCategory.Application, "mspp_entityformmetadata.mspp_attributelogicalname is null.");
					continue;
				}
		...
		protected void PopulateReferenceEntityField(OrganizationServiceContext context, IEntityForm entityform, Control container)
		{
			...
			try
			{
				...
				if (list == null && hiddenField == null)
				{
					ADXTrace.TraceError(TraceCategory.Application, string.Format("'{0}' field is not one of the expected control types.", entityform.TargetAttributeName));
					return;
				}
				...
				{
					ADXTrace.TraceError(TraceCategory.Application, "mspp_entityform.mspp_referenctargetlookupattributelogicalname must not be null.");
					return;
				}

				if (string.IsNullOrWhiteSpace(entityform.ReferenceEntityLogicalName))
				{
					ADXTrace.TraceError(TraceCategory.Application, "mspp_entityform.mspp_referenceentitylogicalname must not be null.");
					return;
				}
				...
				if (!entityform.QuerystringIsPrimaryKey)
				{
					...
					if (entity == null)
					{
						ADXTrace.TraceError(TraceCategory.Application, string.Format("Could not retrieve entity of type '{0}' where '{1}' equals '{2}'.", entityform.ReferenceEntityLogicalName, entityform.ReferenceQueryAttributeName, referenceQueryStringValue));
						return;
					}
					id = entity.Id;
				...
				if (disabled || modalLookup)
				{
					if (text == string.Empty)
					{
						...
						if (string.IsNullOrWhiteSpace(entityform.ReferenceEntityPrimaryKeyLogicalName))
						{
							ADXTrace.TraceError(TraceCategory.Application, string.Format("Error retrieving the Primary Key Attribute Name for '{0}'.", entityform.ReferenceEntityLogicalName));
							return;
						}
						...
						if (entity == null)
						{
							ADXTrace.TraceError(TraceCategory.Application, string.Format("Could not retrieve entity of type '{0}' where '{1}' equals '{2}'.", entityform.ReferenceEntityLogicalName, entityform.ReferenceEntityPrimaryKeyLogicalName, id));
							return;
						}
			...
			{
				ADXTrace.TraceError(TraceCategory.Application, string.Format("PopulateReferenceEntityField: {0}", ex));
			}
		}

		protected void SetEntityReference(OrganizationServiceContext context, Entity entityform, IDictionary<string, object> values)
		{
			var setEntityReference = entityform.GetAttributeValue<bool?>("mspp_setentityreference") ?? false;

			if (!setEntityReference) return;

			var id = Guid.Empty;
			var targetAttributeName = entityform.GetAttributeValue<string>("mspp_referencetargetlookupattributelogicalname") ?? string.Empty;
			var referenceEntityLogicalName = entityform.GetAttributeValue<string>("mspp_referenceentitylogicalname");
			var referenceEntityRelationshipName = entityform.GetAttributeValue<string>("mspp_referenceentityrelationshipname") ?? string.Empty;

			var portalContext = PortalCrmConfigurationManager.CreatePortalContext();
			var referenceEntitySourceType = entityform.GetAttributeValue<OptionSetValue>("mspp_referenceentitysourcetype"); //NEED TO ADD ATTRIBUTE VALUE

			if (string.IsNullOrWhiteSpace(targetAttributeName))
			{
				ADXTrace.TraceInfo(TraceCategory.Application, "Entity Relationship Name is provided. No Entity Reference to set. AssociateEntity will be called during OnInserted event instead.");
				return;
			}
			...
			try
			{
				...
				switch (referenceEntitySourceType.Value)
				{
					case 756150000: // Query String
						var referenceQueryStringName = entityform.GetAttributeValue<string>("mspp_referencequerystringname") ?? string.Empty;
						...
						var querystringIsPrimaryKey = entityform.GetAttributeValue<bool?>("mspp_referencequerystringisprimarykey") ?? false;

						if (!querystringIsPrimaryKey)
						{
							var referenceQueryAttributeName = entityform.GetAttributeValue<string>("mspp_referencequeryattributelogicalname");
							var entity =
					...
					case 756150001: // Record Associated to current user 
						var relationship = entityform.GetAttributeValue<string>("mspp_referencerecordsourcerelationshipname");  
						if (string.IsNullOrWhiteSpace(relationship))
						{
							throw new ApplicationException("Required Relationship Name has not been specified for the Record Source Type 'Record Associated to Current Portal User'.");
						}
		...
		protected void AssociateCurrentPortalUserOnInserting(OrganizationServiceContext context, Entity entityform, CrmEntityFormViewInsertingEventArgs e)
		{
			...
			var bAssociatePortalUser = entityform.GetAttributeValue<bool?>("mspp_associatecurrentportaluser") ?? false;
			var targetEntityLogicalName = entityform.GetAttributeValue<string>("mspp_entityname");
			var portalUserLookupAttributeName = entityform.GetAttributeValue<string>("mspp_targetentityportaluserlookupattribute");
			var portalUserLookupAttributeIsActivityParty = entityform.GetAttributeValue<bool?>("mspp_portaluserlookupattributeisactivityparty") ?? false;

			if (!bAssociatePortalUser) return;

			if (string.IsNullOrWhiteSpace(targetEntityLogicalName))
			{
				ADXTrace.TraceError(TraceCategory.Application, "mspp_entityform.mspp_entityname must not be null.");
				return;
			}

			if (string.IsNullOrWhiteSpace(portalUserLookupAttributeName))
			{
				ADXTrace.TraceWarning(TraceCategory.Application, "mspp_entityform.mspp_targetentityportaluserlookupattribute is null.");
				return;
			}

			if (!MetadataHelper.IsAttributeLogicalNameValid(context, targetEntityLogicalName, portalUserLookupAttributeName))
			{
				ADXTrace.TraceError(TraceCategory.Application, string.Format("'{0}' entity does not contain an attribute with the logical name '{1}'.", targetEntityLogicalName, portalUserLookupAttributeName));
				return;
			}
		...
		{
			var attachFile = entityform.GetAttributeValue<bool?>("mspp_attachfile") ?? false;
